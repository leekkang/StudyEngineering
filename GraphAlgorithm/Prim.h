...
void Prim(std::vector<GraphEdge>& result, const std::vector<GraphNode>& graph, int srcNode = 0) {
	// 노드 개수 확인 == MakeSet
	int nodeCount = graph.size();

	// 트리에 포함된 버텍스 테이블. 트리에 포함되면 해당 인덱스 값이 true가 된다.
	std::vector<bool> vertexTable(nodeCount, false);
	vertexTable[srcNode] = true; // 최초 트리 생성
	int visitCount = 1;
	...
	while(visitCount < nodeCount) {
		...
		if (endNode != -1) {
			...
			result.push_back(GraphEdge(srcNode, endNode, minCost));
		} else {
...
void PrimWithQueue(std::vector<GraphEdge>& result, const std::vector<GraphNode>& graph, int srcNode = 0) {
	// 노드 개수 확인 == MakeSet
	int nodeCount = graph.size();
	...
	const std::vector<std::pair<int, int>>& edges = graph[srcNode].vecEdge;
	// 큐에 간선 추가
	for (int i = 0; i < edges.size(); ++i)
		queue.push(GraphEdge(srcNode, edges[i].first, edges[i].second));
	GraphEdge edge;
